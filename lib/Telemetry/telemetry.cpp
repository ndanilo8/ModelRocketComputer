...
bool Telemetry::send2radio()
{
    ...
    if (timer.hasPassed(timeRate))
    {
        ...
        dtostrf(data.kalman.altitude, 6, 2, buffer); // Leave room for too large numbers!
        strcat(packet, buffer);
        strcat(packet, ",");
        ...
        dtostrf(data.kalman.verticalVelocity, 6, 2, buffer); // Leave room for too large numbers!
        strcat(packet, buffer);
        strcat(packet, ",");
